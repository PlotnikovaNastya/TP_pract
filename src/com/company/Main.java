package com.company;
public class Main {
    public static void main(String[] args) {
        System.out.println("Первая задача:");
        block2_z1 b2_z1 = new block2_z1();
        b2_z1.task1();
        System.out.println("Вторая задача:");
        block2_z2 b2_z2=new block2_z2();
        b2_z2.task2();
        System.out.println("Третья задача:");
        block2_z3 b2_z3=new block2_z3();
        b2_z3.task3();
        System.out.println("Четвертая задача:");
        block2_z4 b2_z4=new block2_z4();
        b2_z4.task4();
        System.out.println("Пятая задача:");
        block2_z5 b2_z5=new block2_z5();
        b2_z5.task5();
        System.out.println("Шестая задача:");
        block2_z6 b2_z6=new block2_z6();
        b2_z6.task6();
        System.out.println("Седьмая задача:");
        block2_z7 b2_z7=new block2_z7();
        b2_z7.task7();
        System.out.println("Восьмая задача:");
        block2_z8 b2_z8=new block2_z8();
        b2_z8.task8();
        System.out.println("Девятая задача:");
        block2_z9 b2_z9=new block2_z9();
        b2_z9.task9();
        System.out.println("Десятая задача:");
        block2_z10 b2_z10=new block2_z10();
        b2_z10.task10();
    }
}
